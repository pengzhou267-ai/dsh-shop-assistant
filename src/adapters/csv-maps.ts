...
export const CSV_ADAPTERS: readonly CsvColumnMap[] = [
  {
    id: 'generic',
    label: '通用（首行即表头）',
    columns: {},
 ...
 */
export function getAdapter(id: string | undefined): CsvColumnMap {
  if (!id) return CSV_ADAPTERS[0]!
...
export function normalizeRow(
  ...
  adapter: CsvColumnMap,
): Record<string, string> {
  if (Object.keys(adapter.columns).length === 0) return { ...row }
